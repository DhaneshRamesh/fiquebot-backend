...
📘 Now, what would you like to ask about Fique?"""
                    }]
                }]
            }
...
async def handle_whatsapp(From: str = Form(...), Body: str = Form(...)):
    ...
    try:
        ...
        import traceback
        traceback.print_exc()
        return PlainTextResponse("⚠️ Sorry, something went wrong.")
...
@app.post("/whatsapp")
async def whatsapp_webhook_alternative(From: str = Form(...), Body: str = Form(...)):
    try:
        async with httpx.AsyncClient() as client:
            response = await client.post("http://localhost:8000/conversation", json={
        ...
        print(f"❌ Error in alternative WhatsApp webhook: {e}")
